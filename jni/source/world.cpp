...
void World::init()
{
    ...
    float angleAdjust = Network::instance->getLocalIdx() * 90.0f + 20;
...
void World::control()
{
    ...
    while (true)
    {
        ...
        if (m_cameraTouch == NULL && t->second.first > 50 && t->second.first <= resolution.first - 200)
        {
    ...
    if (m_cameraTouch != NULL)
    {
        ...
        else
        {
            ...
            m_camera->control(t->second.first - m_cameraLastX, t->second.second - m_cameraLastY);
            m_cameraLastX = t->second.first;
            m_cameraLastY = t->second.second;
        }
    }
