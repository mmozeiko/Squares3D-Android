...
/*
    ...
    const float t = 1.0f / std::tan(fov/2 * DEG_IN_RAD) * 0.1f;
    const float b = -t;
...
*/    
    ...
    const float fov = 45.0f / aspect;
    const float f = 1.0f / std::tan(fov/2 * DEG_IN_RAD);
    ...
    projection.m02 = 0;
    projection.m12 = 0;
    projection.m22 = (fz+nz)/(nz-fz);
    projection.m32 = 2*fz*nz/(nz-fz);
...
Video::Video()
{
    ...
    m_resolution = make_pair(800, 480);
