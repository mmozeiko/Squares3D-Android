...
Language::Language()
{
    LOG("Initializing language texts... ");
...
void Language::reload()
{
    ...
    LOG("loading %s language.", Config::instance->m_misc.language.c_str());

    int selCount = load(Config::instance->m_misc.language);
    if (selCount != count)
    {
        LOG("Warning: expected %i phrases, but found only %i!", count, selCount);
    }
}
