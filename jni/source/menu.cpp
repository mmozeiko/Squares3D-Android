...

{
    ...
    m_backGround.push_back(Face(UV(0.0f, 1.0f - aspect), Vector(), Vector::Zero));
    m_backGround.push_back(Face(UV(0.0f, aspect), Vector(), Vector(0.0f, resY, 0.0f)));
    m_backGround.push_back(Face(UV(1.0f, 1.0f - aspect), Vector(), Vector(resX, 0.0f, 0.0f)));
    m_backGround.push_back(Face(UV(1.0f, aspect), Vector(), Vector(resX, resY, 0.0f)));
...
void Menu::loadMenu(Profile* userProfile, int unlockable, int& current)
{
    ...
    submenu->addEntry(new LabelEntry(this, language->get(TEXT_CREDITS_SCREEN), Font::Align_Left, m_fontSmall));
...
void Menu::control()
{
    ...
    {
        float x = (float)(touches.begin()->second.first);
