...
Intro::Intro() : m_timePassed(0), m_nextState(false), m_ballKicked(false), m_mesh(NULL)
{
    ...
    for (int y = 0; y < PIECE_YCOUNT; y++)
    {
        ...
        for (int x = 0; x < PIECE_XCOUNT; x++)
        {
            ...
            for (int k=0; k<4; k++)
            {
                ...
                float v = 1.0f - (y+uv[k][1])*maxv/PIECE_YCOUNT;
    ...
    matrix = Matrix::translate(Vector(0.2f, -0.5f, 0.0f));
    NewtonBody* floorBody = NewtonCreateBody(m_newtonWorld, floor, matrix.m);
...
void Intro::render() const
{
    ...
    // background
    {
        ...
        const float aspect = static_cast<float>(resY)/static_cast<float>(resX);
        ...
        float buf[] = { 0.0f, 0.0f,    0, (float)resY,
                        0.0f, aspect,  0, 0,
                        1.0f, 0.0f,    (float)resX, (float)resY,
